<template>
  <div class="container pt-3 h-100 d-flex flex-column">
    ...
    <b-card class="shadow-sm">
      <b-alert v-if="!members.length" variant="info" show>
        There are no members for this pool yet.
      </b-alert>
...
import {
  ...
  GetMembersResponse,
  IPools,
} from '@thxnetwork/dashboard/store/modules/pools';
...
export default class Members extends Vue {
  ...
  async getMoreMembers({ pool, page, limit }: GetMembersProps) {
    this.loading = true;
    const response: GetMembersResponse = await this.$store.dispatch(
      'pools/getMembers',
      {
        pool,
        page,
