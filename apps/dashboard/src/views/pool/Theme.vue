    ...
    <b-card class="shadow-sm mb-5">
      <b-form-row>
        <b-col md="6">
          ...
          <p class="text-muted small">
            This background image is shown on the login page users see when
            claiming your crypto or NFT's.
          </p>
          <b-form-file
            @change="onUpload($event, 'backgroundImgUrl')"
            accept="image/*"
          />
        </b-col>
        <b-col>
      ...
      <b-form-row>
        <b-col md="6">
          ...
          <p class="text-muted small">
            This logo image is shown above the login panel users see when
            claiming your crypto or NFT's.
          </p>
          <b-form-file
            @change="onUpload($event, 'logoImgUrl')"
            accept="image/*"
          />
        </b-col>
        <b-col>
...
export default class AssetPoolView extends Vue {
  ...
  get isBrandUpdateInvalid() {
    ...
    const logoUrlIsValid = this.logoImgUrl
      ? isValidUrl(this.logoImgUrl)
      : this.logoImgUrl === '';
