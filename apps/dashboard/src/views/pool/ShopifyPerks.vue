        ...
        <b-row class="mb-3">
            ...
            <b-col class="d-flex justify-content-end">
                <b-button v-b-modal="'modalRewardShopifyCreate'" class="rounded-pill" variant="primary">
                    ...
                    <span class="d-none d-md-inline">Shopify Discount Perk</span>
                </b-button>
                <BaseModalRewardShopifyCreate @submit="listRewards" :id="'modalRewardShopifyCreate'" :pool="pool" />
            </b-col>
        </b-row>
            ...
            <BTable hover :busy="isLoading" :items="rewardsByPage" responsive="lg" show-empty>
                ...
                <template #cell(id)="{ item }">
                    <b-dropdown variant="link" size="sm" no-caret>
                        ...
                        <b-dropdown-item v-b-modal="'modalRewardShopifyCreate' + item.id">Edit</b-dropdown-item>
                        <b-dropdown-item
                            @click="$store.dispatch('shopifyPerks/delete', shopifyPerks[pool._id][item.id])"
                        >
                            Delete
                        </b-dropdown-item>
                    </b-dropdown>
                    <BaseModalRewardShopifyCreate
                        @submit="listRewards"
...
@Component({
    ...
    computed: mapGetters({
        pools: 'pools/all',
        totals: 'shopifyPerks/totals',
        shopifyPerks: 'shopifyPerks/all',
    }),
})
export default class ShopifyPerksView extends Vue {
    ...
    get rewardsByPage() {
        if (!this.shopifyPerks[this.$route.params.id]) return [];
        return Object.values(this.shopifyPerks[this.$route.params.id])
            .filter((reward: TShopifyPerk) => reward.page === this.page)
        ...
        switch (action.variant) {
            case 0:
                for (const id of Object.values(this.selectedItems)) {
                    this.$store.dispatch('shopifyPerks/delete', this.shopifyPerks[this.pool._id][id]);
                }
                break;
            case 1:
