...
import { type TShopifyPerk } from '@thxnetwork/types/index';
...
class ShopifyPerkModule extends VuexModule {
    ...
    @Mutation
    set({ pool, reward }: { reward: TShopifyPerk & { _id: string }; pool: TPool }) {
    ...
    @Action({ rawError: true })
    async create({ pool, payload }: { pool: TPool; payload: TShopifyPerkInputData }) {
    ...
    @Action({ rawError: true })
    async update({ pool, reward, payload }: { pool: TPool; reward: TShopifyPerk; payload: TShopifyPerkInputData }) {
        const formData = prepareFormDataForUpload(payload);
        const { data } = await axios({
            method: 'PATCH',
    ...
    @Action({ rawError: true })
    async listPriceRules({ pool, shop }: { pool: TPool; shop: string }) {
        const { data } = await axios({
            method: 'GET',
