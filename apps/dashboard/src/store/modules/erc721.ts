...
class ERC721Module extends VuexModule {
  ...
  setMetadata(payload: { erc721: TERC721; metadata: TERC721Metadata }) {
    if (!this._all[payload.erc721._id].metadata) {
      return Vue.set(this._all[payload.erc721._id], 'metadata', [
        payload.metadata,
      ]);
    }
    const index = payload.erc721.metadata
      ? payload.erc721.metadata.findIndex(
          (m: TERC721Metadata) => m._id === payload.metadata._id
        ) || 0
      : 0;
  ...
  async readMetadata({
    ...
    const metadata = this._all[erc721._id].metadata.find(
      (m) => m._id === metadataId
    );
  ...
  async read(id: string) {
    ...
    const erc721 = {
      ...
      logoURI:
        data.logoImgUrl ||
        `https://avatars.dicebear.com/api/identicon/${data.address}.svg`,
    };
  ...
  @Action({ rawError: true })
  async uploadMetadataCSV(payload: {
    pool: IPool;
    erc721: TERC721;
    file: File;
  }) {
