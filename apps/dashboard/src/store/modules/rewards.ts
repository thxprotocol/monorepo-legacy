...
class RewardModule extends VuexModule {
    ...
    @Mutation
    clearPool({ pool }: { pool: string }) {
        Vue.delete(this._all, pool);
    }
    ...
    async list({ poolId, page, limit }: RewardListProps) {
        ...
        if (data?.results?.length === 0) this.context.commit('clearPool', { pool: poolId });
    ...
    @Action({ rawError: true })
    async delete({ reward }: { reward: Reward }) {
