<template>
  ...
  >
    <template #card-header>
      NFT
      <i
        class="ml-1 fas fa-archive text-white small"
        v-if="erc721.archived"
      ></i>
    </template>
...
export default class BaseCardERC721 extends Vue {
  ...
  onClick() {
    if (this.erc721.poolId)
      this.$router.push({ path: `/pool/${this.erc721.poolId}/rewards` });
  }

  openTokenUrl() {
    const url = `${chainInfo[this.erc721.chainId].blockExplorer}/token/${
      this.erc721.address
    }`;
