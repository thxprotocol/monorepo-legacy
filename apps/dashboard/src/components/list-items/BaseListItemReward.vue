      ...
      <div class="d-flex align-items-center">
        ...
        <b-dropdown size="sm" variant="white" no-caret right>
          ...
          <b-dropdown-item @click="onEdit()"
            ><i class="fas fa-pen mr-3"></i> Edit reward
          </b-dropdown-item>
...
export default class BaseCardReward extends Vue {
  ...
  get filteredMetadata() {
    return (
      this.erc721 &&
      this.erc721.metadata &&
      this.erc721.metadata.filter((m: TERC721Metadata) => !m.tokenId)
    );
  }
  ...
  mounted() {
    if (this.reward.withdrawCondition) {
      ...
      this.channelAction =
        ChannelAction[this.reward.withdrawCondition.channelAction];
    ...
    if (this.reward.amount == 1) {
      this.$store.dispatch('brands/getForPool', this.pool).then(() => {
        const logoImgUrl = this.brand
          ...
          : BASE_URL + require('../../../public/assets/qr-logo.jpg');
