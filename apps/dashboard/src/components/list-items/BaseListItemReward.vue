...
import { Reward, ChannelType, ChannelAction, RewardState } from '@thxnetwork/dashboard/types/rewards';
    ...
    mounted() {
        ...
        if (this.reward.amount == 1) {
            this.$store.dispatch('brands/getForPool', this.pool).then(() => {
                const logoImgUrl = this.brand ? this.brand.logoImgUrl : BASE_URL + require('../../../public/assets/qr-logo.jpg');
