          ...
          <b-tab title="General" active>
            ...
            <b-form-group class="mb-0">
              <b-form-checkbox v-model="isMembershipRequired">
                ...
                <p>
                  Verifies that the user claiming the reward has a membership
                  for the pool.
                </p>
              </b-form-checkbox>
            </b-form-group>
...
export default class ModalRewardCreate extends Vue {
  ...
  async onRewardChange() {
    ...
    if (this.reward.erc721metadataId) {
      ...
      this.erc721metadata =
        this.erc721?.metadata?.find(
          (meta) => meta._id === this.reward.erc721metadataId
        ) || null;
    } else {
  ...
  onRewardVariantChanged(variant: RewardVariant) {
    switch (variant) {
      case RewardVariant.NFT: {
        this.rewardWithdrawAmount = 0;
        this.rewardWithdrawLimit = 0;
        this.erc721metadata = this.filteredMetadata
          ? this.filteredMetadata[0]
          : null;
        break;
      }
  ...
  async getYoutube() {
    ...
    if (isAuthorized && this.channel) {
      this.warning = '';
      this.channelActions[ChannelAction.YouTubeLike].items =
        this.youtube.videos;
      this.channelActions[ChannelAction.YouTubeSubscribe].items =
  ...
  async getTwitter() {
    ...
    if (isAuthorized && this.channel) {
      this.warning = '';
      this.channelActions[ChannelAction.TwitterLike].items =
        this.twitter.tweets;
      this.channelActions[ChannelAction.TwitterRetweet].items =
        this.twitter.tweets;
      this.channelActions[ChannelAction.TwitterFollow].items =
        this.twitter.users;
    }
  }
  ...
  async onChannelClick(
    channel: IChannel,
    action: IChannelAction | null = null
  ) {
    switch (channel.type) {
  ...
  async submit() {
    this.loading = true;
    const expiryDate =
      this.rewardExpireDate &&
      this.concatDatetime(this.rewardExpireDate, this.rewardExpireTime);
