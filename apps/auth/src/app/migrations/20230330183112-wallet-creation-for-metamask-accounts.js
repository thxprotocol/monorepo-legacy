...
module.exports = {
  async up(db) {
    const accounts = await (await db.collection('accounts').find({ variant: 4 }, { projection: { address: 1 } })).toArray(); //metamask account variant
    const promises = accounts.map(async (account) => {
      const wallets = await getWallet(account._id);
      if (!wallets.length) {
...
async function createWallet(sub, address) {
  const r = await apiClient({
    ...
    data: { sub, chainId, deploy: false, address },
  });
  return r.data;
}
