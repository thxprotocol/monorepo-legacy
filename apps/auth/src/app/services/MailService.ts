...
export class MailService {
  static async sendConfirmationEmail(
    ...
    const html = await ejs.renderFile(
      path.join(__dirname, './assets/views/mail/signupConfirm.ejs'),
      {
        verifyUrl,
        returnUrl,
        baseUrl: AUTH_URL,
    ...
    await this.sendMail(
      account.email,
      'Please complete the sign up for your THX Account',
      html,
      verifyUrl
    );
  ...
  static async sendLoginLinkEmail(account: AccountDocument, password: string) {
    ...
    const html = await ejs.renderFile(
      path.join(__dirname, './assets/views/mail/ginLink.ejs'),
      {
        loginUrl,
    ...
    await this.sendMail(
      account.email,
      'A sign in is requested for your Web Wallet',
      html,
      loginUrl
    );
  ...
  static async sendResetPasswordEmail(
    ...
    const html = await ejs.renderFile(
      path.join(__dirname, './assets/views/mail/resetPassword.ejs'),
      {
        resetUrl,
        returnUrl,
        baseUrl: AUTH_URL,
