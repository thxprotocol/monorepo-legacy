...
import { DiscordGuild, DiscordMessage, DiscordReaction, QuestSocial } from '@thxnetwork/api/models';
...
const controller = async ({ params, body, account }: Request, res: Response) => {
    ...
    if (quest.variant === QuestVariant.Discord) {
        const guild = await DiscordGuild.findOne({ poolId: quest.poolId });
        data.metadata['discordMetrics'] = {
            reactionCount: await DiscordReaction.countDocuments({ guildId: guild.id, userId: platformUserId }),
            messageCount: await DiscordMessage.countDocuments({ guildId: guild.id, userId: platformUserId }),
        };
    }
    ...
    if (quest.variant === QuestVariant.Twitter) {
        ...
        data.metadata['twitterMetrics'] = user.publicMetrics;
    }
