...
const controller = async (req: Request, res: Response) => {
    ...
    const balance = await erc20.contract.methods.balanceOf(account.walletAddress).call();
    if (balance < Number(amount)) throw new InsufficientBalanceError();

    // Check allowance for admin to ensure throughput
    const allowance = Number(
        await erc20.contract.methods.allowance(account.walletAddress, req.assetPool.address).call(),
    );
