<template>
  ...
  >
    ...
    <template v-else>
      ...
      <p>
        You might have pending withdrawals in your emporary wallet. Please
        provide your password to transfer the ownership of your assets to your
        new wallet.
      </p>
...
export default class ModalDecodePrivateKey extends Vue {
  ...
  onShow() {
    const web3 = new Web3();
    this.account = web3.eth.accounts.privateKeyToAccount(
      this.privateKey
    ) as any;
    this.busy = false;
  }
  ...
  async onSubmit() {
    ...
    try {
      const tempPrivateKey = decryptString(
      ...
      if (isPrivateKey(tempPrivateKey)) {
        this.tempAccount =
          this.web3.eth.accounts.privateKeyToAccount(tempPrivateKey);
      } else {
