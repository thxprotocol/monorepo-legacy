...
class MetamaskStore extends VuexModule {
  ...
  setAccount({
    ...
    if (!this.account || !keepAccount || !accounts.includes(keepAccount)) {
      ...
      sessionStorage.setItem(
        'thx:wallet:metamask:currentAccount',
        this.account
      );
    }
  }
  ...
  async approve(payload: {
    ...
    const allowance = await contract.methods
      .allowance(this.account, payload.spender)
  ...
  checkPreviouslyConnected() {
    ...
    const currentAccount = sessionStorage.getItem(
      'thx:wallet:metamask:currentAccount'
    ) as string;
    if (currentAccount) {
  ...
  async sign(payload: { poolAddress: string; method: string; params: any[] }) {
    ...
    const solution = new this.web3.eth.Contract(
      defaultPoolDiamondAbi as any,
      payload.poolAddress,
      {
        from: this.account,
      }
    );
    const abi: any = defaultPoolDiamondAbi.find(
      (fn) => fn.name === payload.method
    );
    const nonce =
      Number(await solution.methods.getLatestNonce(this.account).call()) + 1;
