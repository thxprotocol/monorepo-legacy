...
import {
  isPrivateKey,
  MINIMUM_GAS_LIMIT,
} from '@thxnetwork/wallet/utils/network';
import {
  HARDHAT_RPC,
  NODE_ENV,
  POLYGON_MUMBAI_RPC,
...
import {
  mockPrivateKeyForSubject,
  torusClient,
} from '@thxnetwork/wallet/utils/torus';
...
class NetworkModule extends VuexModule {
  ...
  get chains() {
    return Object.values(chainInfo)
      .filter((chain: ChainInfo) => {
        if (NODE_ENV === 'production' && chain.chainId === ChainId.Hardhat)
          return;
  ...
  async getPrivateKey(user: User) {
    ...
    if (TEST_KEY) {
      this.context.commit(
        'setPrivateKey',
        mockPrivateKeyForSubject(user.profile.sub)
      );
      return;
    }
    ...
    if (!torusKey)
      throw new Error('Could not fetch private key from Torus network');
  ...
  async connect(chainId: ChainId) {
    ...
    const isMetamaskAccount = user
      ? user.profile.variant === AccountVariant.Metamask
      : false;
    const web3 =
  ...
  async sign(msg: string) {
    if (this.privateKey) {
      const hash = '';
      return this.web3.eth.accounts.sign(hash as any, this.privateKey)
        .signature;
    } else {
  ...
  async send({ to, fn }: { to: string; fn: any }) {
    ...
    const gas = String(
      estimate < MINIMUM_GAS_LIMIT ? MINIMUM_GAS_LIMIT : estimate
    );
